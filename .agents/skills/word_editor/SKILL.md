...
Lệnh này sinh ra đồng thời ba file:

1. `word_structure.json`: Chứa cấu trúc JSON cho script ánh xạ tự động. Giữ nguyên 100% mảng gốc (kể cả dòng rỗng). **Không chứa comment** — script mapping không cần đọc comment, chỉ cần `text` gốc.
2. `word_structure.md`: Chứa outline dạng Markdown map 1:1 số lượng dòng với mảng `paragraphs` trong JSON (bao gồm cả các dòng trắng). Có kèm thẻ `<r:...>` cho đoạn text có màu khác biệt và comment marker.
3. `word_structure_compact.md`: Giống hệt file `.md` nhưng **đã lược bỏ toàn bộ các dòng paragraph rỗng** để tiết kiệm tối đa LLM token.

Agent **luôn ưu tiên đọc `.compact.md`** để hiểu nội dung, đọc yêu cầu từ comment và định vị vị trí cần điền thông qua `anchor`. Chỉ khi nào `anchor` không thể phân biệt do trùng lặp dữ liệu, Agent mới fallback sang đọc file `.md` (full) để tự đếm đúng `paragraph_index` nhằm map với `.json`. File `.json` chỉ dùng làm file nguồn để đối chiếu tọa độ, chủ yếu dùng khi viết script ánh xạ sinh `changes.json` để agent có thể tận dụng index thay vì tự viết raw string trong một số trường hợp phù hợp.
...
Word chia mỗi paragraph/cell thành nhiều Run — mỗi Run mang một bộ định dạng riêng. Để tối ưu token và tránh làm nhiễu LLM, script lược bỏ toàn bộ các định dạng in đậm, in nghiêng, gạch chân... Baseline mặc định được coi là chữ không màu (auto/đen).

Nếu một Run có **màu chữ (Font Color)** khác baseline, file `.md` bọc đoạn text đó bằng thẻ `<r:...>...</r>` với chuỗi màu HEX tương ứng (không có dấu `#`):

> Kết nối và định tuyến động với <r:FF0000>các Router của ISP</r>, dùng để kiểm soát truy cập

Các thẻ có cùng mã màu đứng liền kề nhau sẽ được script tự động gộp (merge) lại thành một thẻ liền mạch để đảm bảo không đứt gãy từ khóa tìm kiếm.
...
**Khi ghi đè (`--apply`):** style tại vị trí đang thay được giữ nguyên tự động — Word áp style hiện có của Range lên text mới khi gán `.Text`, không tạo style riêng cho nội dung mới. Nếu `find` vắt qua ranh giới nhiều thẻ `<r>` có màu khác nhau, Word có thể áp toàn bộ text mới theo màu của Run đầu tiên trong vùng thay vì giữ đúng pattern nhiều màu ban đầu — cứ tuân theo Minimum Edit Principle như bình thường, không cần mở rộng phạm vi `find` để né trường hợp này.

Nếu nội dung gốc của tài liệu tình cờ chứa chuỗi `<r:` hoặc `</r>`, script tự escape (chèn khoảng trắng vào giữa) khi xuất `.md` để tránh nhầm lẫn với tag hệ thống.
...
- Run-level style (`<r:...>`): chỉ đọc được trong `--read` (xuất ra `.md`, không có trong `.json`). `Font.Color` dạng "tự động"/theme color không xác định được mã hex cụ thể — coi như baseline, không hiển thị tag màu.
- Script đánh đổi thời gian lấy độ chính xác, file word nặng script có thể tốn đến 1p hoặc hơn là bình thường.
