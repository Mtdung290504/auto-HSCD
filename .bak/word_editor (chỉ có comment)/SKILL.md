...
2. `word_structure.md`: Chứa outline dạng Markdown có cấu trúc phân cấp (các đoạn văn được ghi trực tiếp trên từng dòng và các bảng biểu dạng Markdown Table được đánh số chỉ số hàng/cột trực quan để xem nhanh), **có kèm comment marker nếu vị trí đó có comment**.

Agent nên đọc `.md` để hiểu nội dung, định vị vị trí cần điền, và đọc yêu cầu từ comment (nếu có). File `.json` chỉ dùng khi viết script ánh xạ để sinh `changes.json`.
...
**Tự động thêm hàng khi thiếu chỗ:** nếu `row_index` vượt quá số hàng hiện có của bảng, `--apply` tự gọi `Rows.Add()` để bảng đủ chỗ trước khi ghi, không báo lỗi và không cần rule riêng để "tạo hàng" — chỉ cần trỏ thẳng `row_index` tới vị trí mong muốn. Ví dụ bảng đang có 4 hàng (index 0-3) nhưng khảo sát có 10 thiết bị: cứ tạo rule với `row_index` chạy tới 9, `--apply` sẽ tự thêm 6 hàng còn thiếu. Số hàng đã thêm được log ra (`[INFO] Table[...]: Đã chèn thêm N hàng mới`).

Vì vậy: **không cần gộp nhiều dòng dữ liệu vào chung 1 ô** để né giới hạn số hàng của bảng mẫu — mỗi thiết bị/dòng dữ liệu trong khảo sát nên có 1 hàng riêng trong bảng kết quả, kể cả khi bảng mẫu ban đầu có ít hàng hơn số lượng thực tế.

Lưu ý: hàng mới thêm bằng `Rows.Add()` có thể không kế thừa đầy đủ định dạng phức tạp (border, merge) của các hàng có sẵn — kiểm tra lại bảng sau khi `--apply` nếu bảng mẫu có cấu trúc phức tạp (ví dụ có cột đánh dấu `+` cố định).
